---
sidebar_position: 1
---

# Plans & Billing

Jovyan AI offers flexible subscription plans for individuals and teams. All paid plans include access to all hosted models — no API keys required.

## Plans Overview

| | **Free** | **Pro** | **Pro+** | **Business** |
|---|---|---|---|---|
| **Price** | $0/month | $20/month | $50/month | $100/month |
| **Hosted models** | -- | All models | All models | All models |
| **Bring your own keys** | Yes | Yes | Yes | Yes |
| **ChatGPT login** | Yes | Yes | Yes | Yes |
| **Usage** | -- | 25% more than API | 3× Pro | 7× Pro |
| **Support** | Community | Standard | Priority | Dedicated |

### Free Plan

Use your own API keys from any supported provider (OpenAI, Anthropic, Google, etc.) or log in with your ChatGPT Plus/Pro subscription. No cost, no limits beyond your own API usage.

### Pro Plan ($20/month)

Access all hosted models through Jovyan AI with standard usage. No API key setup needed — just sign in and start working.

### Pro+ Plan ($50/month)

3× the usage of Pro. Includes priority support.

### Business Plan ($100/month)

7× the usage of Pro. Includes dedicated support and team features.

## Free Trial

New users get a **30-day free trial of Pro** — no credit card required. During the trial you have full access to all hosted models with Pro usage limits.

During the trial, the model selector only shows models available on your current plan. Profiles that use a paid-only model are greyed out with an upgrade tooltip, so you always know which models require a subscription.

After the trial ends, your account moves to the Free plan. You can subscribe to any paid plan at any time.

## Available Hosted Models

Paid plans include access to all of these models:

| Provider | Models |
|----------|--------|
| **Anthropic** | Claude Opus 4.6, Claude Sonnet 4.6, Claude Sonnet 4.5, Claude Opus 4.5, Claude Haiku 4.5 |
| **OpenAI** | GPT-5.2, GPT-5.1, GPT-5 Mini |
| **Google** | Gemini 3.1 Pro, Gemini 3 Pro, Gemini 3 Flash, Gemini 2.5 Pro, Gemini 2.5 Flash |
| **Mistral** | Mistral Large 3 |

New models are added regularly. Check the model selector in the extension for the latest list.

## Usage Limits

Paid plans have usage limits that reset periodically. If you reach your limit, requests are paused until the next reset. You can upgrade your plan at any time for more capacity.

## Managing Your Subscription

Visit your [account dashboard](https://jovyan-ai.com/account) to:

- View current plan and usage
- Upgrade or downgrade your plan
- Cancel your subscription
- View billing history

You can also change your plan at any time — upgrades take effect immediately, downgrades apply at the end of the current billing period.

## Related

- [Create Your Account](/quick-start/create-account) - Get started with Jovyan AI
- [Modes & AI Agent](/features/modes-ai-agent) - Choose the right model for your task
